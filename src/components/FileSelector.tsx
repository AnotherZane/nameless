...
const FileSelector = () => {
  const [role, hide] = useSessionStore((ss) => [ss.role, ss.hide]);
  ...
  const [sharedMeta, progress] = useReceiverStore((r) => [
    r.sharedMetadata,
    r.progress,
  ]);
            ...
            {(role == ShareRole.Sender
              ...
              .map(([fid, file]) => {
                const prog = Math.round(
                  ((progress.get(fid) ?? 0) / file.size) * 100
                );
                    ...
                    <ListItemAvatar>
                      <Avatar>
                        <CircularProgressWithIcon
                          ...
                          variant="determinate"
                          value={role == ShareRole.Sender ? 0 : prog}
                        />
                      </Avatar>
                    </ListItemAvatar>
                    <ListItemText
